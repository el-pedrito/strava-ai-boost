...
|----|--------|--------|----------|--------|------------|
| T1 | Forged Strava webhook events trigger processing of arbitrary activity IDs | Spoofing | Medium | ✅ Mitigated | Strava does **not** sign webhook events, so there is no signature to verify. Primary control (2026-07-27): `validate_webhook_origin` drops any event whose `subscription_id` or `owner_id` does not match our subscription/known athletes (validated on 339 real events). Defence in depth: events only reference IDs — data is re-fetched from Strava with the user's OAuth token, so a forged payload cannot inject content, and an unknown activity id just 404s. Residual: `verify_token` (subscription-validation GET only) is predictable and not yet in Secrets Manager — see BACKLOG P1 before public release; webhook has no throttling yet |
