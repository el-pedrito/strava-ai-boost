...
- [ ] **A3. Évals de régression des prompts** — spec écrite + challengée le 2026-07-16 : [design/regression-evals.md](./design/regression-evals.md). **Plan amendé après challenge** : V1 = harnais local on-demand (fixtures synthétiques anonymisées + critères **déterministes** : clichés bannis, dashes, longueur, emojis, JSON parseable) qui invoque **le runtime déployé `content_gen`** (décision utilisateur : tester la vraie chaîne de prod ; isolation memory via `user_id="regression_eval"` dédié + hooks memory déjà désactivés à la génération), ~0,20 $/run, déclenché quand `embedded_prompts.py` change (après déploiement). Signal de tendance vs baseline, **pas un gate CI** (non-déterminisme LLM). Pas de refactor du prompt : test de synchronisation `BANNED_CLICHES` ↔ texte du prompt. Le service managé **AgentCore Evaluations existe** (vérifié doc AWS : custom evaluators LLM-as-a-Judge, on-demand, Strands/OTel, CDK) mais relégué en **V2 vitrine OSS** — pour le besoin mono-user, le juge LLM ajoute coût+bruit vs les checks déterministes. Coach feedback = V2 aussi (contexte 4 semaines lourd à synthétiser).
