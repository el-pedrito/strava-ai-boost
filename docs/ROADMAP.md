...
- **Passe durcissement + hygiène ops (2026-07-27)** — grosse session post-v0.2.1, tout déployé et vérifié en prod, dépôt à `bd5412f`. Commits : `e479ed9` (scripts ambient creds + drift validation), `48571fc` (CVE npm 8 HIGH→0), `e458a35`/`8a18f8f`/`e8afbbc` (ESLint flat config + lint 25 err→0), `536a7fb` (2 défauts responsive), `584c811` (oauth refresh + consolidation token), `309f091` (filtrage origine webhook), `0f1b1d2` (assets Lambda déterministes), `69fc66a` (script deploy frontend), `eafd4a3` (3 scripts ops réparés), `bd5412f` (backlog + corrections claims).
  - **Déploiement complet + ASH** : `cdk deploy --all` (8 stacks `UPDATE_COMPLETE`), scan ASH 3.5.8 → 1 seul finding first-party actionable (`B104` bind `0.0.0.0` du conteneur AgentCore, protégé customJWT). Résultats réels dans `.ash/ash_output/reports/ash.flat.json` (l'agrégat top-niveau est un stub).
  - **CVE npm 8 HIGH → 0** : `postcss` 8.5.23, `eslint` 10, `react-router` 8 (suppression de `react-router-dom` consolidé dans `react-router`, 12 imports réécrits), `react-leaflet` 5 (conflit peer React 19).
  - **Lint frontend réparé** : `npm run lint` cassé depuis toujours (aucun `eslint.config.js`) → config flat. 38 findings exhumés → **0 erreur / 24 warnings**. `SortIcon` hissé hors render (Dashboard+Quality), ref `useAutoRefresh` dans un effet, interfaces vides → alias. 12 warnings `react-hooks` laissés en dette tracée (P3 backlog).
  - **Frontend jamais redéployé depuis le 16 juillet** : le bucket servait un bundle 11 jours en retard (la stack Frontend n'a pas de `BucketDeployment`, `deploy.sh` ne touche pas le frontend, aucun script n'existait). Corrigé par `scripts/deploy_frontend.sh` (build → sync hors `config.json` → invalidation CloudFront + attente → vérif bundle servi==construit). Bundle à jour en prod, vérifié.
  - **2 défauts responsive** trouvés en mesurant les pages authentifiées (via bypass dev `sab-devmock` + injection des données réelles de prod) : tableau Qualité `overflow-hidden` rognait les colonnes à 768px (Dashboard avait déjà `overflow-x-auto`), et le conteneur `FlashToasts` (`w-full max-w-sm right-4`) débordait à gauche à 375px. Corrigés et vérifiés (0 débordement à 375/768/1440). Preuve visuelle : charts Coach affichent `Face pull` en 13 points, tri des tableaux inverse correctement.
  - **OAuth : statut basé sur le refresh token** : `/config/oauth` annonçait « reconnectez-vous » dès l'expiration du token d'accès (durée 6h), donc « déconnecté » la majeure partie de la journée alors que le pipeline se rafraîchit seul. Désormais : expiré + refresh token présent → `connected:true` / `expired_refreshable`. Vérifié en prod (token expiré depuis 6h → connecté).
  - **Consolidation du refresh token** : `shared/strava_oauth.py` était mort ; `activity_fetcher` (avec `datetime.utcnow()` naïf) et `feedback_analyzer` avaient chacun leur copie. Les deux délèguent maintenant au helper partagé, métadonnées timezone-aware partout. Prouvé en prod (refresh live à 15:35, `last_refreshed` tz-aware). 10 tests ajoutés sur ce chemin qui n'en avait aucun.
  - **Filtrage d'origine webhook** (voir bloc sécurité ci-dessous) : `validate_webhook_origin` (subscription_id + owner_id), fail-open sur config absente, réponse 200, coupe-circuit `WEBHOOK_STRICT_ORIGIN`. Validé sur 339 events réels (338/338 acceptés, 1 rejeté). ⚠️ **Incident auto-infligé** : un POST de test forgé a traversé le pipeline (l'ancienne « compensation HMAC » était infondée — Strava ne signe pas), déclenchant l'alarme DLQ à 15:38 ; nettoyé (message DLQ + record DynamoDB supprimés, alarme repassée OK à 15:45).
  - **Assets Lambda déterministes** : `Code.from_asset("lambda_functions")` empaquetait `__pycache__`/`.pyc` (74 fichiers), donc chaque run de tests changeait le hash et `cdk diff` ne pouvait pas dire « déployé==source ». `exclude=[...]` sur les 18 déclarations → hash stable prouvé, `cdk diff` reste à 0 après tests.
  - **3 scripts ops réparés** (`reprocess_dlq`, `configure_strava_webhook`, `cleanup_strava_webhook`) : dérive `--profile your-aws-profile` qui cassait avec des credentials d'environnement → motif `PROFILE_ARGS`. `reprocess_dlq.sh` (outil de récup DLQ) était **inutilisable**, prouvé réparé.
  - **Chemin authentifié validé de bout en bout** : 21/21 endpoints en `COGNITO_USER_POOLS`, rejets 401 (sans token / malformé / JWT forgé), et succès avec un **vrai token Cognito** obtenu par SRP via un utilisateur jetable (supprimé ensuite). CORS restreint au domaine CloudFront réel.
...
- **Tests** : 674 (578 backend unit + 43 régression + 53 frontend, au 2026-07-26).
...
> **maintenant (mis à jour 2026-07-27)** : le durcissement + hygiène ops est fait et déployé (cf. Done du 2026-07-27). Reste, par ordre :
> **(1)** confirmer le filtrage webhook sur une **vraie activité Strava** (contrôle : `aws logs filter-log-events --log-group-name /aws/lambda/StravaAIBoost-WebhookHandler --filter-pattern '"Dropping webhook"'` doit rester vide ; coupe-circuit `WEBHOOK_STRICT_ORIGIN=false` si besoin) — seule preuve que je ne peux pas fabriquer moi-même ;
> **(2)** confirmation visuelle des pages authentifiées derrière OAuth (Coach charts, tableaux triables, responsive) — validé au niveau code/build/bundle servi mais pas avec une session réelle ;
> **(3)** avant de rendre le repo public : durcir le `verify_token` webhook (P1 backlog — aléatoire dans Secrets Manager + lecture par le script + fermeture, dans cet ordre) ;
> **(4)** rendre le repo public (aucun bloquant technique restant, v0.2.0 taguée) → post LinkedIn (`docs/architecture.md`) → backfill PRs Strava (si envie).
> Muscu : ~~peupler les charts~~ ✅ (backfill 2026-07-26) ; ~~normaliser les noms d'exercices~~ ✅ (2026-07-26, validé en prod le 2026-07-27 : `Face pull` **13 séances** affiché en 13 points dans l'UI).
> Scan sensibilité + CVE + licence déjà faits.
> Sécurité (ASH 3.5.8, 2026-07-27) : 1 seul finding first-party actionable — `B104` bind `0.0.0.0` dans `src/coach_chat/coach_chat_agent.py` (requis par le conteneur AgentCore, protégé par customJWT). `CKV_AWS_59` sur l'API webhook = exception assumée (Strava doit l'appeler). ⚠️ Correction 2026-07-27 : la « compensation HMAC-SHA1 » annoncée était infondée — [Strava ne signe pas ses événements webhook](https://developers.strava.com/docs/webhooks/), donc `verify_webhook_signature` empruntait toujours sa branche permissive et un POST anonyme traversait tout le pipeline (démontré : écriture DynamoDB, exécution Step Functions, message DLQ, alarme). Compensation réelle depuis : `validate_webhook_origin` rejette tout événement dont `subscription_id` ou `owner_id` ne correspond pas à l'abonnement et aux athlètes connus, avec réponse 200 (Strava désactive les abonnements qui renvoient des non-200) et coupe-circuit `WEBHOOK_STRICT_ORIGIN`. Validé sur les 339 événements réels du 2026-03-26 au 2026-07-27 : 338/338 légitimes acceptés, 1 seul rejeté (le test forgé). CVE npm : ~~`postcss`, `react-router`, `eslint`~~ ✅ toutes corrigées le 2026-07-27 — `npm audit` passe de 8 HIGH à **0** (`postcss` 8.5.23, `eslint` 10, `react-router` 8 avec suppression de `react-router-dom` désormais consolidé dans `react-router` et réécriture des 12 imports, `react-leaflet` 5 pour lever le conflit de peer React 19 latent). Build + 53 tests frontend verts, routage vérifié en navigateur.
> Lint frontend (2026-07-27) : `npm run lint` était cassé depuis toujours (aucun `eslint.config.js` n'avait jamais existé) → config flat ajoutée. 38 findings préexistants exhumés, ramenés à **0 erreur / 24 avertissements** : `SortIcon` hissé hors du render (Dashboard + Quality), ref de `useAutoRefresh` déplacée dans un effet, interfaces vides `Input`/`Textarea` converties en alias, directive `eslint-disable` obsolète retirée, 3 faux positifs `static-components` (icônes Lucide dynamiques) supprimés de façon ciblée et commentée. Reste en dette tracée volontairement en `warn` : 11 `set-state-in-effect` + 1 `refs` (patterns d'état légitimes mais datés dans auth/onboarding/preferences/media) et les avertissements `exhaustive-deps`/`react-refresh` — refactor à faire dans un changement dédié, testé derrière authentification.
> Responsive (2026-07-27) : audit statique + mesures navigateur réelles à 375 / 768 / 1440 px sur la page de connexion — aucun scroll horizontal ni débordement ; tableaux Dashboard/Quality ont déjà une alternative mobile (`hidden md:block` / `md:hidden`), 11 `ResponsiveContainer` pour les graphiques. Pages authentifiées restant à vérifier avec la session utilisateur.
