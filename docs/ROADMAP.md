...
- [ ] **Endpoint de déautorisation Strava** — **confirmé** (changelog 1 juin 2026). Flow « Déconnecter Strava » : bouton + dialog confirmation → `POST /strava/disconnect` (Cognito) → `POST /oauth/deauthorize` Strava + effacement tokens Secrets Manager + `strava_connected=false`. Gérer aussi le webhook déautorisation entrant. ⚠️ **destructif** (tokens) : idempotent, ne supprime **pas** les données d'activités, réversible via re-OAuth. Test live bloqué (app Inactive) → tests unitaires mock.
