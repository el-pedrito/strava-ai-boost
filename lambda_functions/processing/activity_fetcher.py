...
# Markers that identify our OWN generated output (assembled description/signature).
# Used to avoid feeding a previous generation back as the athlete's "original" text
# on a reprocess (that self-reinforcing loop let a generation mangle, e.g.
...
def _looks_generated(text: Optional[str]) -> bool:
    """True if the text carries one of our generated-output signatures."""
    return any(marker in (text or '') for marker in _GENERATED_MARKERS)


def _resolve_original_text(
    ...
    If the fetched Strava description is our own generated output (carries a
    signature), keep the first-seen stored original. Otherwise the fetched text is
    authoritative: it is the athlete's original on first processing, or a manual
    edit the athlete made on Strava (which must be honored on reprocess).
    """
    if not _looks_generated(fetched_description):
        return fetched_description, fetched_name
    stored = existing_item or {}
    stored_desc = stored.get('original_description')
    if stored_desc and not _looks_generated(stored_desc):
        stored_name = stored.get('original_name')
        name = stored_name if (stored_name and not _looks_generated(stored_name)) else fetched_name
        return stored_desc, name
    return fetched_description, fetched_name
...
def store_activity_data(
    ...
    try:
        ...
        # Preserve the athlete's TRUE original text across reprocesses (see
        # _resolve_original_text). On a reprocess Strava's current description IS
        # our own generated output; feeding it back as 'original_description' is a
        # self-reinforcing loop. A manual edit by the athlete (no signature) is
        # still honored.
        ...
        if _looks_generated(original_description):
            try:
                ...
                existing_item = {}
        resolved_description, resolved_name = _resolve_original_text(
            original_description, original_name, existing_item
        )
        if resolved_description != original_description:
            logger.info(
                f"Reprocess of {activity_id}: fetched description is our own output; "
                "preserving stored original text instead of overwriting it"
            )
        original_description, original_name = resolved_description, resolved_name
